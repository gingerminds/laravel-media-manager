...
Office Open XML formats are zip archives, and the mime type `getMimeType()` reports comes from libmagic sniffing the zip's internal file listing — a heuristic a plain, unrelated `.zip` upload can occasionally trip, coming back labelled e.g. `...spreadsheetml.sheet`. `store()` passes `MimeTypeNormalizer::normalize()` the upload's real path (read *before* `storeAs()` moves it) so it can double-check that the specific marker file each OOXML variant requires (`xl/workbook.xml` for xlsx, `word/document.xml` for docx, `ppt/presentation.xml` for pptx) actually exists in the zip before trusting the map; if it doesn't, the file is stored as `application/zip` instead.
