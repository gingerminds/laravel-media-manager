 ...
 *
 * Split into cooperating modules (same spirit as gingerminds-laravel-cms's
 * components/blocks/ folder): generic helpers in utils.js, the category
 * `<select>` flattening logic in category-options.js, and the result
 * card/chip DOM building in render.js — this file only owns the class
 * itself (state, event wiring, fetching).
 */
import { Modal } from 'bootstrap';
import { debounce, escapeHtml, extractCollection } from './utils.js';
import { buildCategoryTreeOptions } from './category-options.js';
import { renderCard, renderChip } from './render.js';
...
class MediaSelect {
    constructor(root) {
        ...
        this.previewEl.querySelectorAll('[data-chip]').forEach((chip) => {
            ...
            this.selected.set(id, {
                ...
                language_isos: chip.dataset.languages ? chip.dataset.languages.split(',').filter(Boolean) : [],
            });
        });
    ...
    toggle(card) {
        ...
        if (this.pending.has(id)) {
            ...
            this.pending.set(id, {
                ...
                language_isos: card.dataset.languages ? card.dataset.languages.split(',').filter(Boolean) : [],
            });
        }
    ...
    renderResults(items, reset) {
        ...
        items.forEach((item) => fragment.appendChild(renderCard(item, this.config)));
    ...
    renderPreview() {
        ...
        this.selected.forEach((item) => this.previewEl.appendChild(renderChip(item, this.config)));
    }
...
// `media` type fields, the same way it already does for wysiwyg fields.
export function initMediaSelectFields(scope = document) {
...
document.addEventListener('DOMContentLoaded', () => initMediaSelectFields());
